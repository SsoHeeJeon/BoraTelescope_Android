...
public class Visitmanager : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        if (gamemanager.navibarstate == GameManager.NavibarState.Open || gamemanager.navibarstate == GameManager.NavibarState.Opening)
        {
            gamemanager.navibarstate = GameManager.NavibarState.Closing;
        }

        if (gamemanager.langbarstate == GameManager.LangBarState.Open && gamemanager.langbarstate == GameManager.LangBarState.Opening)
        {
            gamemanager.langbarstate = GameManager.LangBarState.Closing;
        }
        gamemanager.modeonoff.VisitMode.transform.GetChild(0).gameObject.SetActive(true);
    ...
    public void OnClickStickerBtn()
    {
        ...
        PencilColor.SetActive(false);
        BackGroundColor.SetActive(false);
        StickerColor.transform.GetChild(0).GetComponent<RectTransform>().anchoredPosition = new Vector2(0, 0);
    }
    ...
    [SerializeField] GameObject PencilImg;
