...
public class NamSanHMode : MonoBehaviour
{
    ...
    [SerializeField]
    GameObject InfoHeight;
    [SerializeField]
    RectTransform DetailTxtCotent;
    // Start is called before the first frame update
    void Start()
    {
        gamemanager = GameObject.Find("GameManager").GetComponent<GameManager>();
        gamemanager.namsanhmode = this;
        ...
        gamemanager.modeonoff.NamsanHIn();
        ...
        if (gamemanager.navibarstate != GameManager.NavibarState.Open)
        {
            gamemanager.navibarstate = GameManager.NavibarState.Opening;
        }
        gamemanager.ChangeLang(GameManager.langstate);
    }
    ...
    public void ReadyTo360(GameObject type)
    {
        ...
        gamemanager.langbarstate = GameManager.LangBarState.Closing;
        gamemanager.etcbarstate = GameManager.EtcBarState.Closing;
        ...
        {
            case 1:
                DetailImage_Content.sizeDelta = new Vector2(484, 320);
                break;
            case 2:
                DetailImage_Content.sizeDelta = new Vector2(945, 320);
                break;
            case 3:
                DetailImage_Content.sizeDelta = new Vector2(1410, 320);
                break;
            case 4:
                DetailImage_Content.sizeDelta = new Vector2(1875, 320);
                break;
        }
        Detailname(num - 1);
        if (GameManager.langstate == GameManager.LangState.Korea)
        {
            InfoHeight.GetComponent<UIText>().text = ReadJsonFile.DetailText_K[num + 4];
        ...
        else if (GameManager.langstate == GameManager.LangState.English)
        {
            InfoHeight.GetComponent<UIText>().text = ReadJsonFile.DetailText_E[num + 4];
        ...
        else if (GameManager.langstate == GameManager.LangState.Chienses)
        {
            InfoHeight.GetComponent<UIText>().text = ReadJsonFile.DetailText_C[num + 4];
            Narration.clip = gamemanager.Narration_Docent_C[num - 1];
            Docent_avartar.transform.GetComponent<DocentAni>().NarrtionLen = Narration.clip.length;
            Docent_avartar.transform.GetComponent<DocentAni>().AdClip = Narration.clip;
        }
        else if (GameManager.langstate == GameManager.LangState.Japan)
        {
            InfoHeight.GetComponent<UIText>().text = ReadJsonFile.DetailText_J[num + 4];
        ...
        scroll.DisObject();
        Invoke("LabelDetailSizeSetting", 0.5f);
    }

    void LabelDetailSizeSetting()
    {
        DetailTxtCotent.anchoredPosition = new Vector2(DetailTxtCotent.anchoredPosition.x, 0);
        DetailTxtCotent.sizeDelta = new Vector2(DetailTxtCotent.sizeDelta.x, InfoHeight.GetComponent<RectTransform>().sizeDelta.y + 5);
    }
    ...
    public void NarrStopPlay(GameObject btn)
    {
        if (!scroll.ChromaVideo.activeSelf)
        {
