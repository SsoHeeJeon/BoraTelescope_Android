...
public class See360NH : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        {
            if(namsanHMode.Docent_avartar.activeSelf)
            {
                if(Input.GetTouch(0).position.x<1500)
                {
                    if(namsanHMode.gamemanager.etcbarstate != GameManager.EtcBarState.Close || namsanHMode.gamemanager.langbarstate != GameManager.LangBarState.Close)
                    {
